...
function(datos,nsim=99){
   ...
   tableZ=cbind(ONt[order(ONt[,1]),], ENt[order(ENt[,1]),3],St[order(St[,1]),3],
                round(Z.obs,2),round(pZas,4))
   ...
   tablaC=data.frame(cbind(df,round(c(C.obs,Ci.obs),2),round(c(pCas,pCias),4)))
   ...
   return (list(ON=ON, EN=EN, Z=Z.obs, S=S, pZas=pZas,pZr=pZr,
                C=C.obs, Ci=Ci.obs, pCas=pCas, pCias=pCias,
