`plot.ecespa.kci` <-
function(x, type=1, q=0.025, kmean=TRUE, add=F,
     ...
     lty=c(1,2,3), col=c(1,2,3), lwd=c(1,1,1), ...)
{
   ...
   if (type == 1) {                      ## univariado t<ed>pico L1
       cosa <- cbind(x$kia, 
                  t(apply(x$kia.s,1,quantile, c(q,1-q),na.rm=T)))
       cosamean <- sqrt(apply(x$kia.s,1,mean,na.rm=T)/pi)- x$r
   ...
   if (type == 2) {                      ## univariado t<ed>pico L2
       cosa <- cbind(x$kib, 
                  t(apply(x$kib.s,1,quantile, c(q,1-q),na.rm=T)))
       cosamean <- sqrt(apply(x$kib.s,1,mean,na.rm=T)/pi)- x$r
       cosa <- sqrt(cosa/pi)-x$r
   ...
   if (type == 12) {                      ## bivariado t<ed>pico L12
       cosa <- cbind(x$kci.ab.o, 
                  t(apply(x$kci.ab.s,1,quantile, c(q,1-q),na.rm=T)))
       cosa <- sqrt(cosa/pi)-x$r
       cosamean <- apply(x$kci.ab.s,1,mean,na.rm=T)
   ...
   if (type == 21) {                       ## bivariado t<ed>pico L21
       cosa <- cbind(x$kci.ba.o, 
                  t(apply(x$kci.ba.s,1,quantile, c(q,1-q),na.rm=T)))
       cosa=sqrt(cosa/pi)-x$r
       cosamean <- apply(x$kci.ba.s,1,mean,na.rm=T)
       cosamean <- sqrt(cosamean/pi)-x$r
